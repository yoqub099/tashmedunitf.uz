...
/** Strip legacy sections from every language variant */
...
const ALL_FIELDS: FieldConfig[] = [
  ...
  { name: "duration", label: "O'qish muddati", type: "text", placeholder: "4 yil" },
...
export default function BakalaviatDirectionDetailPage() {
  const params = useParams();
  const id = Number(params.id);
  const { data: directionData, isLoading, refetch } = useDirectionDetail(id);
  const { data: siblingsData } = useDirections({ per_page: 50, degree: "bakalavriat" });
      ...
      <Container className="py-6">
        ...
        {otherDirections.length > 0 && (
          <div className="rounded-2xl p-4 md:p-6 lg:rounded-3xl mt-6 bg-gray-100">
            <h4 className="font-serif text-2xl font-semibold mb-6">Boshqa bakalavriat yo&apos;nalishlari</h4>
            <div className="grid gap-3 sm:grid-cols-2">
              {otherDirections.map((d) => {
                const DIcon = iconFor(d.code);
                return (
                  <Link key={d.id} href={`/abiturientlarga/bakalavriat/${d.id}`} className="flex items-center gap-3 rounded-2xl bg-white p-4 sm:p-5 hover:shadow-md transition-shadow duration-300 group">
