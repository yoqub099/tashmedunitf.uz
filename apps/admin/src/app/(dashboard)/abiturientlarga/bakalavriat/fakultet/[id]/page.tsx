...
export default function BakalaviatFacultyDetailPage() {
  const params = useParams();
  const router = useRouter();
  const id = Number(params.id);
  const { data: faculty, isLoading: fLoading } = useFacultyDetail(id);
  const { data: directionsData, isLoading: dLoading, error: dError, refetch: refetchDirections } = useDirections({ per_page: 50, degree: "bakalavriat", faculty_id: id });
  ...
  const handleCreateFaq = useCallback(async (formData: FormData) => {
    ...
    await createFaq.mutateAsync({
      ...
      answer: { uz: answerUz || "", ru: answerRu || "", en: answerEn || "" },
      category: "faculty",
      faculty_id: id,
  ...
  const handleCreate = useCallback(async (formData: FormData) => {
    formData.set("level", "bakalavriat");
  ...
  const faqs = (faqsData?.data || []).sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));

  return (
    <div className="space-y-4 sm:space-y-6 pb-10 sm:pb-16">
      {/* ═══════════ Section 1 — Hero Grid ═══════════ */}
      <Container className="pt-4 sm:pt-8">
        <section className="grid gap-4 sm:gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 flex flex-col">
            {/* Breadcrumb */}
            <nav className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm text-gray-500 mb-3 sm:mb-5 overflow-x-auto scrollbar-none">
              <Link href="/abiturientlarga" className="hover:text-[#00575B] transition-colors shrink-0">Abiturientlarga</Link>
              <ChevronRight className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-gray-400 shrink-0" />
              <Link href="/abiturientlarga/bakalavriat" className="hover:text-[#00575B] transition-colors shrink-0">Bakalavriat</Link>
              <ChevronRight className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-gray-400 shrink-0" />
              <span className="text-gray-900 font-medium truncate max-w-32 sm:max-w-64">{facultyName}</span>
            </nav>
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {faculty?.image ? (
                <div className="mt-4 sm:mt-6 relative overflow-hidden rounded-xl sm:rounded-2xl lg:rounded-3xl group/img">
                  <img
                    src={faculty.image}
                    alt={facultyName}
                    className="w-full h-auto rounded-xl sm:rounded-2xl lg:rounded-3xl object-cover aspect-video"
                  />
                  {/* Image controls overlay */}
                  <div className="absolute inset-0 bg-black/0 group-hover/img:bg-black/40 transition-all duration-300 rounded-xl sm:rounded-2xl lg:rounded-3xl flex items-center justify-center gap-2 sm:gap-3 opacity-0 group-hover/img:opacity-100">
                    <button
                ...
                <div
                  onClick={() => setIsFacultyEditOpen(true)}
                  className="mt-4 sm:mt-6 relative overflow-hidden rounded-xl sm:rounded-2xl lg:rounded-3xl bg-gray-100 border-2 border-dashed border-gray-300 hover:border-[#00575B] hover:bg-[#00575B]/5 transition-all cursor-pointer group aspect-video flex flex-col items-center justify-center gap-2 sm:gap-3"
                >
                  ...
                  <div className="text-center px-4">
                    <p className="text-xs sm:text-sm font-medium text-gray-500 group-hover:text-[#00575B] transition-colors">Fakultet rasmini yuklang</p>
        ...
        <Container>
          <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 lg:rounded-3xl bg-gray-100">
            <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-5">
              ...
              <h2 className="font-serif text-lg sm:text-2xl font-semibold">Fakultet tavsifi</h2>
            </div>
            <div
        ...
        <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 lg:rounded-3xl bg-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
            <div className="flex items-center gap-2.5 sm:gap-3">
              ...
              <div>
                <h2 className="font-serif text-lg sm:text-2xl font-semibold">Yo&apos;nalishlar</h2>
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-full bg-[#00575B] px-4 py-2 sm:px-5 sm:py-2.5 text-xs sm:text-sm font-medium text-white hover:bg-[#004a4e] transition-colors cursor-pointer shadow-sm hover:shadow-md w-fit"
            >
              <Plus className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
              <span className="hidden sm:inline">Yo&apos;nalish qo&apos;shish</span>
              ...
              {directions.map((dir, idx) => {
                ...
                return (
                  <EditableWrapper key={dir.id} entityType="direction" entityId={dir.id} onEdit={() => setEditItem(dir)} onDelete={() => setDeleteId(dir.id)} label="Yo'nalish">
                    <div onClick={() => router.push(`/abiturientlarga/bakalavriat/${dir.id}`)} className="rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 bg-white flex flex-col gap-2.5 sm:gap-3 h-full shadow-sm hover:shadow-md transition-shadow duration-300 cursor-pointer group">
                      {/* Icon + Title */}
                      <h5 className="font-serif text-base sm:text-lg font-semibold flex items-start gap-2.5 sm:gap-3 text-[#00575B]">
                      ...
                      {/* Description */}
                      {dir.description?.uz && (
                        <div className="text-xs sm:text-sm text-gray-500 leading-relaxed line-clamp-3 sm:line-clamp-4 prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: sanitizeHtml(dir.description.uz) }} />
                      )}
      ...
      {/* ═══════════ Section 4 — FAQ ═══════════ */}
      <Container>
        <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 lg:rounded-3xl bg-gray-50">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="rounded-lg sm:rounded-xl bg-[#00575B]/10 p-2 sm:p-2.5">
                <MessageCircleQuestion className="h-4 w-4 sm:h-5 sm:w-5 text-[#00575B]" />
              </div>
              <div>
                <h2 className="font-serif text-lg sm:text-2xl font-semibold">FAQ</h2>
                <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
                  {faqs.length > 0
                    ? `${faqs.length} ta savol mavjud`
                    : "Tez orada qo'shiladi"}
                </p>
              </div>
            </div>
              ...
              {faqs.map((faq) => {
                ...
                return (
                  <EditableWrapper key={faq.id} entityType="faq" entityId={faq.id} onEdit={() => setEditFaq(faq)} onDelete={() => setDeleteFaqId(faq.id)} label="FAQ">
                    <div className="rounded-xl sm:rounded-2xl bg-white transition-shadow duration-300 hover:shadow-sm">
                      <button
                        onClick={() => setOpenFaqId(isOpen ? null : faq.id)}
                        className="flex w-full items-center justify-between px-4 py-3.5 sm:px-6 sm:py-5 text-left gap-3"
                      >
                        <h6 className="text-sm sm:text-base font-semibold leading-tight text-gray-900 lg:text-lg">
                          {faq.question?.uz || "Savol"}
                        </h6>
                        <ChevronDown
                      ...
                      {isOpen && (
                        <div className="px-4 pb-4 sm:px-6 sm:pb-5">
                          <div
                            ...
                            dangerouslySetInnerHTML={{ __html: sanitizeHtml(faq.answer?.uz) }}
                          />
                        </div>
                      )}
                    </div>
      ...
      <EditModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Yangi yo'nalish qo'shish" fields={DIRECTION_FIELDS} initialData={{ level: "bakalavriat", faculty_id: String(id), is_active: true, sort_order: 0 }} onSubmit={handleCreate} isLoading={createDirection.isPending} />
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          router.push("/abiturientlarga/bakalavriat");
        }}
