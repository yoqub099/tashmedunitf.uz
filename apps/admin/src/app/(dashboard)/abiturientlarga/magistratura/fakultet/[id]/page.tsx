...
export default function MagistraturaFacultyDetailPage() {
  const params = useParams();
  const router = useRouter();
  const id = Number(params.id);
  const { data: faculty, isLoading: fLoading } = useFacultyDetail(id);
  const { data: directionsData, isLoading: dLoading, error: dError, refetch: refetchDirections } = useDirections({ per_page: 50, degree: "magistratura", faculty_id: id });
  ...
  const handleCreate = useCallback(async (formData: FormData) => {
    formData.set("level", "magistratura");
  ...
  return (
    <div className="space-y-6 pb-16">
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 flex flex-col">
            ...
            <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-5">
              ...
              <Link href="/abiturientlarga/magistratura" className="hover:text-[#00575B] transition-colors">Magistratura</Link>
              <ChevronRight className="h-3.5 w-3.5 text-gray-400" />
              ...
              {directions.map((dir, idx) => {
                ...
                return (
                  <EditableWrapper key={dir.id} entityType="direction" entityId={dir.id} onEdit={() => setEditItem(dir)} onDelete={() => setDeleteId(dir.id)} label="Yo'nalish">
                    <div onClick={() => router.push(`/abiturientlarga/magistratura/${dir.id}`)} className="rounded-2xl p-5 md:p-6 bg-white flex flex-col gap-3 h-full shadow-sm hover:shadow-md transition-shadow duration-300 cursor-pointer group">
                      {/* Icon + Title */}
      ...
      <EditModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Yangi yo'nalish qo'shish" fields={DIRECTION_FIELDS} initialData={{ level: "magistratura", faculty_id: String(id), is_active: true, sort_order: 0 }} onSubmit={handleCreate} isLoading={createDirection.isPending} />
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          router.push("/abiturientlarga/magistratura");
        }}
