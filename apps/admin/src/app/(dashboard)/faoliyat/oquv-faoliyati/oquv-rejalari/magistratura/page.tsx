...
export default function MagistraturaOquvRejalariPage() {
  const { data: facultiesData, isLoading } = useFaculties({ level: "magistratura", per_page: 50 });
  ...
  return (
    <section className="py-8 sm:py-12">
      <Container>
        ...
        {faculties.length === 0 ? (
          <p className="mt-8 text-gray-500">Magistratura fakultetlari topilmadi.</p>
        ) : (
      ...
      <EditModal
        ...
        initialData={editFaculty ? { ...editFaculty, level: "magistratura" } : { level: "magistratura", is_active: true }}
        onSubmit={async (formData) => {
          formData.append("level", "magistratura");
          if (editFaculty) {
      ...
      <ConfirmDialog
        ...
        message="Haqiqatan ham bu fakultetni o'chirmoqchimisiz?"
      ...
      <EditModal
        ...
        initialData={editDirection ? { ...editDirection, level: "magistratura" } : { level: "magistratura", faculty_id: createDirectionFacultyId, is_active: true }}
        onSubmit={async (formData) => {
          formData.append("level", "magistratura");
