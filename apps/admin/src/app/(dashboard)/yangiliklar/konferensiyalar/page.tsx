...
const TITLE = "Konferensiyalar";
const BADGE_LABEL = "Konferensiya";
const SIDEBAR_TITLE = "So'ngi konferensiyalar";
...
export default function KonferensiyalarPage() {
  ...
  // Single query — fetch 18 items to reuse first 8 for grid + first 10 for sidebar
      ...
      <Container>
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Ilmiy konferensiyalar va seminarlar</p>
          </div>
        ...
        {isLoading ? (
          ...
          <EmptyState title="Konferensiyalar topilmadi" message="Hozircha konferensiya qo'shilmagan" action={{ label: "Yangi qo'shish", onClick: () => setIsCreateOpen(true) }} />
        ) : (
          <>
              ...
              {featured && (
                <div className="news-card-shadow group relative flex w-full flex-col items-start gap-2 rounded-2xl bg-gray-50 p-4 md:p-6 lg:rounded-3xl">
                  ...
                  <div className="flex w-full items-center gap-2 pb-0.5 pt-2">
                    <CrimsonBadge />
                    <span className="ml-auto text-sm font-extrabold text-gray-900">00:00:00</span>
                  </div>
        ...
        {/* Modals */}
        <EditModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Yangi konferensiya qo'shish" fields={KONF_FIELDS} initialData={{ category: CATEGORY, is_published: true }} onSubmit={handleCreate} isLoading={createNews.isPending} />
        {editItem && editDetail && !isDetailLoading && (
          <EditModal isOpen={!!editItem} onClose={() => setEditItem(null)} title="Konferensiyani tahrirlash" fields={KONF_FIELDS}
        ...
        <ConfirmDialog isOpen={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Konferensiyani o'chirish" message="Bu konferensiya butunlay o'chiriladi. Davom etasizmi?" isLoading={deleteNews.isPending} />
      </Container>
    </section>
  );
}
