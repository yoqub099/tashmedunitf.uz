...
const TITLE = "Tadbirlar";
const BADGE_LABEL = "Tadbir";
const SIDEBAR_TITLE = "So'ngi tadbirlar";
...
const TADBIR_FIELDS: FieldConfig[] = [
...
/* ═══════════════════════════════════════════
   ISFT Sub-components (Crimson + Green theme)
   ═══════════════════════════════════════════ */
...
export default function TadbirlarPage() {
  ...
  // Main list fetches more items (18) so we can reuse first 8 for grid and next 10 for sidebar — saves 1 API call
  const { data, isLoading, error, refetch } = useNews({ page, per_page: 18, category: CATEGORY });
  ...
  const items = allItems.slice(0, 8); // main grid — 8 ta
  const latestItems = allItems.slice(0, 10); // sidebar — eng so'nggi 10
      ...
      <Container>
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Universitetdagi tadbirlar va voqealar</p>
          </div>
        ...
        {isLoading ? (
          ...
          <EmptyState title="Tadbirlar topilmadi" message="Hozircha tadbir qo'shilmagan" action={{ label: "Yangi qo'shish", onClick: () => setIsCreateOpen(true) }} />
        ) : (
          <>
        ...
        {/* Modals */}
        <EditModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Yangi tadbir qo'shish" fields={TADBIR_FIELDS} initialData={{ category: CATEGORY, is_published: true }} onSubmit={handleCreate} isLoading={createNews.isPending} />
        {editItem && editDetail && !isDetailLoading && (
          <EditModal isOpen={!!editItem} onClose={() => setEditItem(null)} title="Tadbirni tahrirlash" fields={TADBIR_FIELDS}
        ...
        <ConfirmDialog isOpen={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Tadbirni o'chirish" message="Bu tadbir butunlay o'chiriladi. Davom etasizmi?" isLoading={deleteNews.isPending} />
      </Container>
    </section>
  );
}
