/**
 * ============================================================
 * CLIENT ERROR REPORTER (admin) — /look kuzatuv markazi
 * ============================================================
 *
 * Admin paneldagi JS xatolarini backend'ga yuboradi:
 *   POST {API}/client-errors  { source: "admin", message, stack, ... }
 *
 * Admin tokeni (cookie) bo'lsa, xato shu foydalanuvchiga bog'lanadi.
...
// admin base = .../api/v1  →  client-errors: .../api/v1/client-errors
const ENDPOINT = `${RAW_API.replace(/\/$/, "")}/client-errors`;
const SOURCE = "admin";
...
/** Admin token (cookie) — xatoni foydalanuvchiga bog'lash uchun. */
function authHeaders(): Record<string, string> {
  try {
    const m = document.cookie.match(/(?:^|;\s*)admin-token=([^;]+)/);
    if (m && m[1]) return { Authorization: `Bearer ${decodeURIComponent(m[1])}` };
  } catch {
    // ignore
  }
  return {};
