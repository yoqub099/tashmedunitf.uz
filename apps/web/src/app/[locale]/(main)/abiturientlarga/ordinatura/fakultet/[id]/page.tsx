...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildFacultyMetadata(id, "ordinatura");
}

export default async function OrdinaturaFacultyPage({ params }: PageProps) {
  const { id } = await params;
  const data = await loadFacultyPage(id, "ordinatura");
  if (!data) return null;
