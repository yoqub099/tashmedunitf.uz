...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata("murojaatlar-tartibi", { path: "/biz-haqimizda/murojaatlar-tartibi", locale: lang });
}

export default async function MurojaatlarTartibiPage() {
  const lang = await getLanguage();
  let title = s("murojaatlar.default_title", lang);
  let content = "";

  try {
    const res = await getPageBySlug("murojaatlar-tartibi");
    if (res.data) {
      title = t(res.data.title, lang) || title;
      content = t(res.data.content, lang) || "";
    }
  } catch {
    /* page not created yet — show defaults */
  }

  const fallbackContent = `<p>${s("murojaatlar.fallback_p1", lang)}</p>
<p>${s("murojaatlar.fallback_p2", lang)}</p>
<p>${s("murojaatlar.fallback_p3", lang)}</p>
<p>${s("murojaatlar.fallback_p4", lang)}</p>
<p>${s("murojaatlar.fallback_p5", lang)}</p>`;
      ...
      <Container as="main" className="py-6">
        ...
        <Breadcrumb
          items={[
            ...
            { label: s("nav.murojaatlar_tartibi", lang) },
          ]}
          className="mt-3"
        />
