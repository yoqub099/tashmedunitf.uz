...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata("oquv-rejalari-magistratura", { path: "/faoliyat/oquv-faoliyati/oquv-rejalari/magistratura", locale: lang });
}
...
export default async function MagistraturaOquvRejalariPage() {
  const lang = await getLanguage();

  const facultiesRes = await getFaculties({ per_page: 50, level: "magistratura" }).catch(() => ({ success: false, data: [] }));
