...
// TdTUTF Termiz coordinates
const TDTUTF_LAT = 37.2242;
const TDTUTF_LNG = 67.2784;
...
export default function LocationSection() {
  ...
  return (
    <section className="mt-10 lg:mt-20">
      <Container>
        <div className="grid min-h-64 sm:min-h-80 md:min-h-[503px] gap-4 md:grid-cols-2">
          {/* Contact Info Card */}
          <div className="rounded-2xl p-4 md:p-6 lg:rounded-3xl bg-gray-100 lg:p-10">
            <h3 className="font-serif text-2xl font-semibold lg:text-[32px]">
              {s("common.location", language)}
            </h3>
          ...
          {/* Leaflet Map */}
          <div className="relative z-0 rounded-2xl lg:rounded-3xl h-full min-h-80 overflow-hidden" style={{ isolation: 'isolate' }}>
            <LeafletMap
              lat={TDTUTF_LAT}
              lng={TDTUTF_LNG}
              zoom={15}
              popupText={siteConfig.name}
            />
          </div>
        </div>
      </Container>
