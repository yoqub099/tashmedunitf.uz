...
const SOCIAL_MAP: Record<string, { name: string; icon: () => React.JSX.Element }> = {
  social_linkedin:  { name: "LinkedIn",  icon: LinkedInIcon },
  social_facebook:  { name: "Facebook",  icon: FacebookIcon },
  social_instagram: { name: "Instagram", icon: InstagramIcon },
  social_telegram:  { name: "Telegram",  icon: TelegramIcon },
  social_youtube:   { name: "YouTube",   icon: YoutubeIcon },
};

async function getSocialLinks(): Promise<{ name: string; icon: () => React.JSX.Element; href: string }[]> {
  try {
    const res = await api.get<{ success: boolean; data: { key: string; value: { uz: string } }[] }>("/v1/site-contents/social", { tags: ["site-contents"] });
    const items = Array.isArray(res.data) ? res.data : [];
    return items
      .filter((item) => SOCIAL_MAP[item.key] && item.value?.uz)
      .map((item) => ({
        ...SOCIAL_MAP[item.key],
        href: item.value.uz,
      }));
    ...
    // Fallback — hardcoded
    return [
      { ...SOCIAL_MAP.social_telegram, href: siteConfig.links.telegram },
      { ...SOCIAL_MAP.social_instagram, href: siteConfig.links.instagram },
      { ...SOCIAL_MAP.social_facebook, href: siteConfig.links.facebook },
      { ...SOCIAL_MAP.social_youtube, href: siteConfig.links.youtube },
    ];
  }
}
...
export default async function Footer() {
  ...
  const footerNavigation = getFooterNavigation(lang);
        ...
        <div className="flex flex-col gap-y-8 py-6 lg:pt-12">
          <div className="grid w-full grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 lg:gap-x-10">
            ...
            <div className="col-span-2 md:col-span-1">
              <Link href={`/${lang}`} className="mt-1 font-serif text-4xl font-bold lg:mt-2 lg:text-5xl text-[#00575B]">
                <span className="select-none">{siteConfig.shortName}</span>
              </Link>
              <p className="mt-6 text-gray-600">
                {s("footer.tagline", lang)}
              </p>
            </div>
          ...
          <div className="border-t pt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-between text-gray-500">
            <p className="text-center sm:text-left">
              ...
              <Link href={`/${lang}`} className="hover:text-[#00575B] transition-colors">
                {siteConfig.url.replace("https://", "")}
              </Link>
              , <span>{s("footer.copyright", lang)}</span>
            </p>
            <AccessibilityFooterLink serverLang={lang as Language} />
          </div>
        </div>
      </Container>
    </footer>
